...
Alice, 1, 85 Bob,2, 75 Charlie,3,60 Dwayne, 4, 90 Ellen,5,65 Fitz,6,50 Garret,7,80 Hubert,8,90 Isabel,9,100 Jane,10,70
  */
    ...
    function getStudentScoreF() public view returns(uint, Student[] memory) {
        ...
        Student[] memory studentsScoreF = new Student[](studentNumber);
        uint index = 0;
        for (uint i=0; students.length > i; i++) {
            if(students[i].score < 60) {
                studentsScoreF[index] = students[i];
                index++;
            } else {
                index += 0;
            }
    ...
    function sClass() public view returns(Student[] memory) {
        ...
        //지역변수로 배열 사용하려면 배열에 길이를 지정 해주어야함
    ...
    // 학생 전체를 가져오는게 아니라 점수랑 학생 전체 정보말고 
    // 점수랑 뭐 인덱스나 요소 하나만 가져오는 방법 있는지 고민해보기
    ...
    function pushStudent() public {
        //배열을 메모리로 선언하고 개별 요소를 할당해야 함.
        //바로 배열을 인풋 넣으려고 하니까 입력이 안됨.
        string[] memory classList = new string[](1);
        classList[0] = "a";
        setStudent("min",100,classList);
        setStudent("kin",100,classList);
        setStudent("jin",40,classList);
        setStudent("lin",40,classList);
        setStudent("ain",60,classList);
        setStudent("din",60,classList);
        setStudent("cin",50,classList);
        setStudent("qin",50,classList);
    }
